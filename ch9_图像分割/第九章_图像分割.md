...
&emsp;&emsp;对于一般的分类==CNN网络==，如VGG和Resnet，都会在网络的最后加入一些全连接层，经过softmax后就可以获得类别概率信息。但是这个概率信息是1维的，即==只能标识整个图片的类别，不能标识每个像素点的类别==，所以这种全连接方法不适用于图像分割。
